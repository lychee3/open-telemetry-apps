...
class HttpPlugin extends core_1.BasePlugin {
    ...
    _traceClientRequest(request, options, span) {
        ...
        api_1.diag.debug('_traceClientRequest return request');
        return request;
    }
    _incomingRequestFunction(original) {
        ...
        return function incomingRequest(event, ...args) {
            ...
            api_1.diag.debug('%s plugin incomingRequest', plugin.moduleName);
            if (utils.isIgnored(pathname, plugin._config.ignoreIncomingPaths, (e) => api_1.diag.error('caught ignoreIncomingPaths error: ', e))) {
    ...
    _outgoingRequestFunction(original) {
        ...
        return function outgoingRequest(options, ...args) {
            ...
            if (utils.isIgnored(origin + pathname, plugin._config.ignoreOutgoingUrls, (e) => api_1.diag.error('caught ignoreOutgoingUrls error: ', e))) {
            ...
            api_1.diag.debug('%s plugin outgoingRequest', plugin.moduleName);
            api_1.context.bind(request);
    ...
    _startHttpSpan(name, options) {
        ...
        if (requireParent === true && currentSpan === undefined) {
            ...
            span = api_1.NOOP_TRACER.startSpan(name, options);
        }
    ...
    _safeExecute(span, execute, rethrow) {
        try {
            ...
            api_1.diag.error('caught error ', error);
        }
    }
}
