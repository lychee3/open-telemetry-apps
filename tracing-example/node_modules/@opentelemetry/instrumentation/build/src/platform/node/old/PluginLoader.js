    ...
    load(plugins) {
        if (this._hookState === HookState.UNINITIALIZED) {
            ...
            if (requiredModulesToHook.length > 0) {
                api_1.diag.warn(`Some modules (${requiredModulesToHook.join(', ')}) were already required when their respective plugin was loaded, some plugins might not work. Make sure the SDK is setup before you require in other modules.`);
            }
            ...
            RequireInTheMiddle(modulesToHook, (exports, name, baseDir) => {
                ...
                if (modulesToIgnore === DISABLE_ALL_PLUGINS) {
                    api_1.diag.info(`PluginLoader#load: skipped patching module ${name} because all plugins are disabled (${exports.ENV_PLUGIN_DISABLED_LIST})`);
                    return exports;
                }
                if (modulesToIgnore.includes(name)) {
                    api_1.diag.info(`PluginLoader#load: skipped patching module ${name} because it was on the ignore list (${exports.ENV_PLUGIN_DISABLED_LIST})`);
                    return exports;
                }
                api_1.diag.info(`PluginLoader#load: trying to load ${name}@${version}`);
                if (!version)
                    return exports;
                api_1.diag.debug(`PluginLoader#load: applying patch to ${name}@${version} using ${modulePath} module`);
                // Expecting a plugin from module;
                try {
                    const plugin = modulePlugin !== null && modulePlugin !== void 0 ? modulePlugin : require(modulePath).plugin;
                    if (!utils.isSupportedVersion(version, plugin.supportedVersions)) {
                        api_1.diag.warn(`PluginLoader#load: Plugin ${name} only supports module ${plugin.moduleName} with the versions: ${plugin.supportedVersions}`);
                        return exports;
                    }
                    if (plugin.moduleName !== name) {
                        api_1.diag.error(`PluginLoader#load: Entry ${name} use a plugin that instruments ${plugin.moduleName}`);
                        return exports;
                    }
                    ...
                    api_1.diag.error(`PluginLoader#load: could not load plugin ${modulePath} of module ${name}. Error: ${e.message}`);
                    return exports;
                }
            });
            ...
            api_1.diag.error('PluginLoader#load: Currently cannot re-enable plugin loader.');
        }
        else {
            api_1.diag.error('PluginLoader#load: Plugin loader already enabled.');
        }
        return this;
    }
    /** Unloads plugins. */
